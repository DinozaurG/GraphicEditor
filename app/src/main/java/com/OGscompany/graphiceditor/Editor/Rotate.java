...
public class Rotate{
	private static Bitmap rotatePrep(Bitmap bitin, Bitmap bitout, int angle) {
    ...
    public static Bitmap rotateOnAngle(Bitmap bitin, int angle) {
	    int ANG, angle0 = angle;
        while (angle0 < 0){
            angle += 360;
        }
        while (angle0 >= 360){
        ...
        for(; angle0 > 0; angle0 -= 45) {
            if (angle0 >= 45)
                ANG = 45;
            else
                ANG = angle0;
            bitin = rotatePrep(bitin, bitout, ANG);
